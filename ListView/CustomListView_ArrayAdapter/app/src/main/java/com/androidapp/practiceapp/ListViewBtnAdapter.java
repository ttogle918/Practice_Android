...
public class ListViewBtnAdapter extends ArrayAdapter implements View.OnClickListener {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final Button button1 = (Button) convertView.findViewById(R.id.btn_add);

        button1.setOnClickListener(new Button.OnClickListener() {
            public void onClick(View v) {
                if (!listViewItem.isSelected()) {
                    textTextView.setText(Integer.toString(pos + 1) + "번 아이템 선택.");
                    button1.setText("선택 해제하기");
                    listViewItem.setSelected(true);
                } else {
                    textTextView.setText(Integer.toString(pos + 1) + "번 아이템입니다.");
                    button1.setText("선택 하기");
